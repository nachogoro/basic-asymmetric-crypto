...
def split_message_in_blocks(msg, n, base):
    if type(msg) is str:
        ...
        explain(f'To transform a string using modulo {n}, we must split it '
                f'into blocks of at most {block_size}')
    ...
    if type(msg) is int:
        explain(f'To transform a number using modulo {n}, we have to guarantee that it is a number between 0 and {n-1}')
        if msg < n:
        ...
        explain(f'The method we\'ll use is converting the number into a string base {base} and split that in blocks')
        explain(f'NOTE: This may not be the way it is expected to be done in the subject')
        msg_str = encodingtools.get_as_string(msg, n)
        blocks = split_message_in_blocks(msg_str, n, base)
        explain('Now we must convert those blocks of string back into integers')

        result = []
        for block in blocks:
            result.append(encodingtools.get_as_number(block, base))
...
def split_cryptogram_in_blocks(msg, n, base):
    if type(msg) is str:
        ...
        if len(msg) <= padded_block_size:
            explain(f'Since {msg} is only {len(msg)} characters long, it does '
                    'not need to be split into blocks, since it is a single cipherblock')
            return [msg]

        explain(f'We need to split the cryptogram {msg} into blocks of {padded_block_size} characters')
    ...
    if type(msg) is int:
        ...
        msg_as_string = encodingtools.get_as_string(msg, base)
        ...
        explain(f'We need to split the cryptogram {msg_as_string} into blocks of {padded_block_size} characters')
        blocks = split_in_blocks_of_size(msg, padded_block_size)
        explain(f'{msg_as_string} is split into {blocks}')
        explain('Now, we encode each chunk as an number again')

        result = [encodingtools.get_as_number(block, base) for block in blocks]
        return result
...
def assemble_cryptogram(blocks, n, base, cache=None):
    ...
    if type(blocks[0]) is int:
        ...
        as_str = [encodingtools.get_as_string(block, base, cache=cache) for block in blocks]
        ...
        return encodingtools.get_as_number(str_result, base, cache)
...
def assemble_message(blocks, n, base, cache=None):
    ...
    if type(blocks[0]) is str:
        ...
        block_size = get_block_size(n, base)
        explain(f'We pad all blocks (but the last one) adding \'A\' to the left up to {block_size} characters')
...
def assemble_signature(blocks, n, base):
    if type(blocks[0]) is Signed_Pair:
